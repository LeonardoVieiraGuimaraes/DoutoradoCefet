...
Esta seção apresenta os resultados obtidos através das simulações computacionais \cite{morettin2010}. Incluem-se tabelas, gráficos e análises quantitativas sobre a manifestação do Teorema Central do Limite e a validação dos Intervalos de Confiança.
...
Esta seção discute os resultados obtidos, relacionando-os com a teoria estatística \cite{fisher1925} e interpretando as implicações práticas dos achados. Analisamos a manifestação do Teorema Central do Limite e a robustez dos Intervalos de Confiança em contextos de populações não-normais.
...
A comparação entre $n = 5$ e $n = 50$ revela o impacto significativo do tamanho amostral na precisão e na normalidade da distribuição das médias. Visualmente, os histogramas mostram que para $n = 5$, a distribuição das médias ainda exibe uma cauda direita pronunciada; para $n = 50$, a distribuição aproxima-se de uma curva de sino simétrica.

Quantitativamente, a redução na assimetria de 0.8982 para 0.2901 representa uma diminuição de 67.70\%. A variância da distribuição das médias também é afetada pela fórmula $\text{Var}(\overline{X}) = \frac{\sigma^2}{n}$: com $n = 5$, $\text{Var}(\overline{X}) = 5$; com $n = 50$, $\text{Var}(\overline{X}) = 0.5$. A variância diminui em um fator de 10, melhorando a precisão das estimativas.
...
\begin{enumerate}
  ...
  \item \textbf{O IC é robusto mesmo para população não-normal:} A taxa de cobertura observada (0.9324) valida o uso de Intervalos de Confiança baseados na distribuição $t$ mesmo quando a população original não segue uma distribuição normal. 
  
  Isto é de extrema importância prática, pois na maioria dos problemas reais, a normalidade da população não pode ser garantida.
\end{enumerate}
