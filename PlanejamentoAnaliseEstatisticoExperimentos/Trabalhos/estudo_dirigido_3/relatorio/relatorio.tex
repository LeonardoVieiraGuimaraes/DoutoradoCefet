...
Esta subseção apresenta o fundamento teórico mais importante da estatística inferencial: como a distribuição das médias amostrais converge para a normalidade independentemente da distribuição original da população.
...
\begin{itemize}
    ...
    \item \textbf{Justificação de Métodos Paramétricos}: O TCL justifica teoricamente o uso de métodos paramétricos (que assumem normalidade) mesmo quando a população não é normal, desde que a amostra seja suficientemente grande \cite{Montgomery2016}.
...
Esta subseção aborda como construir intervalos que contêm o parâmetro populacional verdadeiro com nível de confiança pré-especificado, fornecendo uma medida de incerteza para estimadores.
...
\begin{equation}
H_1: \text{Pelo menos um } \tau_i \neq 0 \quad \text{(equivalentemente: Pelo menos uma média de grupo é diferente)}
\end{equation}
...
Esta subseção descreve a origem, características e qualidade dos dados utilizados. Compreender os dados é fundamental para validar a adequação das técnicas estatísticas escolhidas.
...
Seguem os detalhes técnicos essenciais sobre a origem e composição dos dados:
...
Os dados apresentam as seguintes características descritivas, que os tornam especialmente apropriados para validar a robustez dos métodos estatísticos quando aplicados a distribuições que violam pressupostos de normalidade:

\input{resultados/tabelas_latex/tab_01_estatisticas_descritivas.tex}
...
Esta subseção documenta as tecnologias utilizadas para executar as análises, garantindo reprodutibilidade e transparência.
...
As bibliotecas complementares utilizadas para testes e cálculos estatísticos avançados incluem:

\begin{itemize}
    \item \textbf{Statsmodels}: Modelos estatísticos avançados e testes específicos.
\end{itemize}
...
Esta subseção apresenta um framework de 4 componentes aplicado a cada técnica, garantindo consistência entre métodos.
...
Os resultados da validação do Teorema Central do Limite são apresentados na seção de Resultados.

\subsubsection{Intervalo de Confiança (IC)}

A análise de Intervalo de Confiança segue a mesma estrutura padronizada. Os componentes são descritos a seguir:
...
Os resultados da validação do Intervalo de Confiança são apresentados na seção de Resultados.

\subsubsection{Análise de Variância (ANOVA)}

A ANOVA é implementada seguindo a estrutura padronizada de 4 componentes. Seguem os detalhes:

\textbf{\large{Componente 1: Configuração e Parâmetros}}

A Tabela a seguir sintetiza os parâmetros essenciais para execução da análise ANOVA:
...
Esta subseção sintetiza os principais parâmetros utilizados em cada análise realizada.

Os resultados da validação da Análise de Variância são apresentados na seção de Resultados.

\subsection{Parâmetros Configurados}

Esta subseção descreve o processamento dos dados e os critérios para tomada de decisões estatísticas.

Os principais parâmetros utilizados em cada análise foram detalhados nas seções anteriores de configuração específica para cada técnica (TCL, IC e ANOVA).
...
Esta subseção documenta como os resultados podem ser reproduzidos de forma independente através do código disponível.
...
\input{resultados/tabelas_latex/tab_02_tcl_resultados.tex}
...
\input{resultados/tabelas_latex/tab_03_ic_resumo.tex}
...
\input{resultados/tabelas_latex/tab_04_anova_estatisticas_grupo.tex}
