...
\begin{titlepage}
    ...
    {\large Programa de Pós-Graduação em Modelagem Matemática e Computacional}\\[3.5cm]
    ...
    {\normalsize Disciplina de Tópicos em Teoria dos Grafos}\\[0.3cm]
    {\normalsize Professor: Thiago de Souza Rodrigues}\\[4cm]
...
\subsubsection{Implementação do Algoritmo de Força Bruta}

O algoritmo de força bruta implementado neste trabalho emprega técnicas de \textbf{backtracking com poda} para reduzir drasticamente o espaço de busca, tornando viável a execução em grafos com até 15--20 vértices. A estratégia adotada difere da força bruta ingênua (que testaria todas as $k^n$ atribuições) ao incorporar as seguintes otimizações:

\begin{enumerate}
    \item \textbf{Ordenação por Grau Decrescente}: Os vértices são processados em ordem decrescente de grau. Vértices de alto grau possuem mais restrições de adjacência, e colori-los primeiro maximiza as oportunidades de poda precoce de ramos inválidos.
    
    \item \textbf{Backtracking Recursivo}: Para cada número de cores $k = 1, 2, 3, \ldots$, o algoritmo tenta colorir os vértices um por um recursivamente. Ao detectar um conflito (nenhuma cor disponível para o vértice atual), o algoritmo retrocede imediatamente (\textit{backtrack}), abandonando aquele ramo da árvore de busca.
    
    \item \textbf{Poda por Conflito Imediato}: Antes de atribuir uma cor a um vértice, o algoritmo verifica quais cores já foram usadas pelos vizinhos. Apenas cores válidas (não utilizadas por vizinhos) são testadas, evitando explorações desnecessárias.
    
    \item \textbf{Pré-Computação de Vizinhanças}: As listas de adjacência são pré-calculadas em um dicionário para acesso em tempo constante $O(1)$ durante a recursão, acelerando a verificação de conflitos.
\end{enumerate}

\textbf{Complexidade Resultante:} Embora a complexidade teórica do pior caso permaneça exponencial $O(k^n)$, as otimizações reduzem significativamente o fator constante e o número de ramos explorados na prática. Para grafos com $n \leq 15$ e densidade moderada ($p = 0.3$), o algoritmo executa em tempo razoável (segundos a minutos), enquanto grafos maiores tornam-se rapidamente inviáveis.

\subsubsection{Geração de Instâncias e Coleta de Dados}
...
Após gerar as instâncias, coletamos sistematicamente as métricas de desempenho e resultados obtidos. Para cada instância, foram registrados:
...
A Tabela \ref{tab:parametros_grafos_2} resume o tamanho e a densidade das instâncias aleatórias utilizadas na força bruta, deixando explícito como o aumento de $n$ e da densidade impacta o espaço de busca. Já a Tabela \ref{tab:resultados_forca_bruta_2} destaca o número cromático ótimo e os tempos médios por tamanho, evidenciando onde o algoritmo começa a ficar impraticável.
...
A Figura \ref{fig:escalabilidade} deixa clara a explosão combinatória: a curva dispara após $n=13$, alinhando o achado experimental à complexidade teórica exponencial.

\textbf{Observações:}

\begin{itemize}
    \item Para $n = 5$: tempo médio $\approx 3.96 \times 10^{-5}$ segundos (0,04 milissegundos, desprezível)
    \item Para $n = 10$: tempo médio $\approx 8.14 \times 10^{-5}$ segundos (0,08 milissegundos)
    \item Para $n = 20$: tempo médio $\approx 8.032$ mil segundos por instância (2,23 horas)
    \item Crescimento exponencial extremamente pronunciado na escala logarítmica
    \item Variabilidade aumenta dramaticamente com $n$, indicando forte dependência da estrutura do grafo
\end{itemize}

A curva evidencia a \textbf{inviabilidade prática} do método exato para $n > 13$. Conforme demonstrado experimentalmente, instâncias com $n = 13$ consumiram em média 8,55 milissegundos, $n = 15$ exigiu em média 153 milissegundos, e $n = 20$ consumiu em média 8.032 mil segundos (2,23 horas), tornando o método totalmente impraticável para instâncias maiores.
...
A Figura \ref{fig:numero_cromatico} mostra que o número cromático cresce de forma quase linear na faixa testada, reforçando que densidade e tamanho determinam conjuntamente o valor de $\chi(G)$ nos grafos $G(n,p)$.

\textbf{Análise:}

\begin{itemize}
    \item O número cromático cresce de forma aproximadamente linear com $n$
    \item Para grafos com $p = 0.3$, observa-se $\chi(G)$ variando entre 0.40$n$ (para $n=5$) e 0.63$n$ (para $n=20$), mostrando que a razão $\chi(G)/n$ aumenta ligeiramente com o tamanho do grafo
...
As Figuras \ref{fig:n10_i01}--\ref{fig:n10_i03} ilustram colorações ótimas distintas para instâncias de mesmo tamanho, destacando como variações na conectividade alteram o número de cores e a distribuição cromática.
...
A Tabela \ref{tab:informacoes_dimacs_2} descreve as cinco instâncias DIMACS reais, permitindo relacionar grau médio e densidade com o custo de coloração observado nas seções seguintes.
...
A Tabela \ref{tab:resultados_welsh_powell_2} detalha o desempenho da Welsh-Powell: tempos em milissegundos e quantidade de cores, evidenciando a viabilidade da heurística mesmo nos grafos mais densos.

\textbf{Destaques:}

\begin{itemize}
    \item A instância \textbf{e} (4\,730 vértices) foi resolvida em apenas 99 milissegundos, tempo absolutamente inviável para força bruta
...
As Figuras \ref{fig:inst_a}--\ref{fig:inst_e} evidenciam como densidade e grau médio moldam a coloração: grafos mais densos (\textbf{b}, \textbf{e}) concentram mais conflitos e exigem mais cores, enquanto \textbf{d} permanece parcimonioso mesmo com quase dois mil vértices.
...
A Figura \ref{fig:cores_instancia} compara diretamente o número de cores entre as cinco instâncias, reforçando que a estrutura (densidade) domina o tamanho na determinação de $\chi(G)$.
...
A Figura \ref{fig:tempo_heuristica} mostra que todos os casos permanecem abaixo de 50 ms, confirmando a escalabilidade polinomial da heurística para diferentes tamanhos e densidades.

A Figura \ref{fig:tempo_heuristica} demonstra a eficiência da heurística: mesmo a maior instância (4\,730 vértices) foi processada em aproximadamente 99 milissegundos. A escala logarítmica permite comparar tempos que variam de 2,1 ms (instância \textbf{a}) a 99,0 ms (instância \textbf{e}), evidenciando o crescimento polinomial característico do algoritmo $O(n \log n + m)$.
...
A Figura \ref{fig:vertices_cores} deixa evidente a ausência de correlação simples entre $|V|$ e o total de cores; a topologia local (grau médio) responde pela maior parte da variabilidade observada.
...
A Figura \ref{fig:densidade_cores} evidencia a relação entre densidade e número cromático: grafos mais densos tendem a requerer mais cores, pois possuem mais restrições de adjacência. Embora a densidade seja um fator relevante, o número de cores depende também de outras propriedades estruturais como o grau médio e a distribuição de graus. A análise das 5 instâncias DIMACS demonstra que grafos com densidade similar podem apresentar números cromáticos diferentes, indicando que a estrutura local do grafo é determinante.
...
\begin{enumerate}
    ...
    \item \textbf{Heurística Welsh-Powell}: Não garante otimalidade, mas produz soluções de boa qualidade em tempo polinomial. Para as instâncias testadas, a heurística Welsh-Powell foi aproximadamente 23.000 vezes mais rápida que a força bruta (527,53 s / 0,02293 s), tornando evidente o impacto catastrófico da diferença entre complexidade exponencial e polinomial.
    
    \item \textbf{Qualidade da Heurística}: Embora não possamos comparar diretamente com soluções ótimas nas instâncias grandes, a heurística encontrou colorações razoáveis. Por exemplo, na instância \textbf{d} (densidade 0.68\%, grau médio 13.05), apenas 10 cores foram necessárias para um grafo com 1\,916 vértices e 12\,506 arestas. Este resultado demonstra a eficácia da heurística em grafos esparsos, onde a baixa conectividade permite colorações econômicas.
...
A Tabela \ref{tab:comparacao_algoritmos_2} consolida, lado a lado, tempos, escalas e qualidade das soluções, facilitando a leitura rápida do trade-off entre exatidão e eficiência.

O algoritmo de Força Bruta, testado em 48 grafos (5-20 vértices), garantiu soluções ótimas com tempo médio de 527,53 segundos por instância, consumindo aproximadamente 7 horas de processamento computacional total. A complexidade exponencial confirmada empiricamente torna este método viável apenas para validação teórica e problemas de escala muito reduzida — instâncias com mais de 13 vértices tornam-se computacionalmente inacessíveis.

Em contraste, a heurística Welsh-Powell demonstrou eficiência excepcional ao processar as 5 instâncias DIMACS (450-4\,730 vértices) em tempo médio de apenas 22,93 milissegundos. A maior instância, com 4\,730 vértices e 286\,722 arestas, foi colorida em 99 milissegundos, evidenciando empiricamente a complexidade polinomial $O(n \log n + m)$ do algoritmo. Este desempenho torna Welsh-Powell a única alternativa viável para aplicações práticas em grafos reais.
