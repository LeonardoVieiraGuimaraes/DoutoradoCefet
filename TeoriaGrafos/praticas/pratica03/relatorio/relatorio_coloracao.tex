...
Esta seção apresenta os conceitos fundamentais necessários para a compreensão do problema de Coloração de Vértices em Grafos. Inicialmente, são introduzidas as definições básicas de grafos e suas propriedades estruturais. Em seguida, formalizamos o problema de coloração de vértices, estabelecendo limites teóricos para o número cromático. Por fim, descrevemos os dois métodos implementados neste trabalho: o algoritmo de força bruta, que garante a solução ótima mas possui complexidade exponencial, e a heurística de Welsh-Powell, que oferece soluções aproximadas em tempo polinomial.

\subsection{Grafos e Representações}

Um grafo $G$ é definido como um par ordenado $G = (V, E)$, onde:
\begin{itemize}
...
\noindent Este trabalho utiliza grafos \textbf{não-direcionados} (onde $(u,v) = (v,u)$, ou seja, arestas sem orientação), \textbf{simples} (sem loops ou arestas múltiplas entre o mesmo par de vértices) e \textbf{não-ponderados} (onde cada aresta possui peso unitário). Essas características simplificam o modelo e permitem focar no problema de coloração \cite{boaventura2002grafos}.
...
Para qualquer grafo $G$, valem as seguintes relações \cite{feofiloff2011introduccao}:
...
Como contraposição ao método exato, apresentamos agora a heurística de Welsh-Powell, que obtém soluções de boa qualidade em tempo polinomial através de uma estratégia gulosa.

A heurística de Welsh-Powell é um algoritmo guloso que ordena os vértices por grau decrescente e atribui cores sequencialmente \cite{boaventura2002grafos}. O algoritmo opera da seguinte forma:
...
As Tabelas \ref{tab:parametros_grafos} e \ref{tab:resultados_forca_bruta} apresentam os dados estruturais das instâncias geradas e os resultados computacionais obtidos.

\input{tabelas/tabela_parametros_grafos.tex}

\input{tabelas/tabela_resultados_forca_bruta.tex}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{img/parte1_grafico1_escalabilidade_tempo.png}
...
\begin{itemize}
    \item Para $n = 5$: tempo médio $\approx 4.3 \times 10^{-5}$ segundos (desprezível)
    \item Para $n = 10$: tempo médio $\approx 0.002$ segundos (2 milissegundos)
    \item Para $n = 13$: tempo médio $\approx 4$ segundos
...
A curva evidencia a \textbf{inviabilidade prática} do método exato para $n > 13$. Extrapolando os dados, uma instância com $n = 15$ levaria aproximadamente 500 segundos ($\sim$8 minutos), enquanto $n = 20$ exigiria vários anos de processamento.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{img/parte1_grafico2_numero_cromatico.png}
...
\begin{itemize}
    ...
    \item Este comportamento está alinhado com a teoria: grafos aleatórios $G(n,p)$ tendem a ter $\chi(G) \approx \frac{n}{2\log_{1/(1-p)}n}$ para $p$ fixo \cite{feofiloff2011introduccao}
...
Para ilustrar o funcionamento do algoritmo de força bruta, apresentamos três instâncias aleatórias com $n = 13$ vértices e probabilidade de aresta $p = 0.3$. As Figuras \ref{fig:n13_i01}, \ref{fig:n13_i02} e \ref{fig:n13_i03} mostram as colorações ótimas encontradas pelo algoritmo exato.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/grafo_n13_i01.png}
    \caption{Coloração ótima: instância $n=13$ (exemplo 1)}
    \label{fig:n13_i01}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/grafo_n13_i02.png}
    \caption{Coloração ótima: instância $n=13$ (exemplo 2)}
    \label{fig:n13_i02}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/grafo_n13_i03.png}
    \caption{Coloração ótima: instância $n=13$ (exemplo 3)}
    \label{fig:n13_i03}
\end{figure}
...
Para demonstrar a eficácia prática da heurística, utilizamos cinco instâncias DIMACS reais de tamanhos variados (450 a 4\,730 vértices). As características estruturais dessas instâncias são apresentadas na Tabela \ref{tab:info_instancias}. Os resultados são apresentados em tabelas estruturadas, seguidas de visualizações gráficas e análises comparativas que destacam o desempenho superior em relação à força bruta.

\input{tabelas/tabela_informacoes_instancias.tex}

\subsubsection{Resultados Obtidos}

A Tabela \ref{tab:resultados_heuristica} apresenta os resultados da aplicação da heurística nas cinco instâncias de grande porte.

\input{tabelas/tabela_resultados_heuristica.tex}

\textbf{Destaques:}

\begin{itemize}
    \item A instância \textbf{e} (4\,730 vértices) foi resolvida em apenas 51,3 milissegundos, tempo inviável para força bruta
    \item A instância \textbf{d}, apesar de ter 1\,916 vértices, requer apenas 10 cores devido à sua baixíssima densidade (0.68\%)
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/instancia_a.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/instancia_b.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/instancia_c.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/instancia_d.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/instancia_e.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/parte2_grafico1_cores_por_instancia.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/parte2_grafico2_tempo_execucao.png}
    \caption{Tempo de execução por instância (escala logarítmica)}
    \label{fig:tempo_heuristica}
\end{figure}

A Figura \ref{fig:tempo_heuristica} demonstra a eficiência da heurística: mesmo a maior instância (4\,730 vértices) foi processada em aproximadamente 51 milissegundos. A escala logarítmica permite comparar tempos que variam de 1,3 ms (instância \textbf{a}) a 51,3 ms (instância \textbf{e}), evidenciando o crescimento polinomial característico do algoritmo $O(n \log n + m)$.

\subsubsection{Relação entre Vértices e Cores}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{img/parte2_grafico3_vertices_vs_cores.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{img/parte2_grafico4_densidade_vs_cores.png}
...
Este trabalho apresentou uma análise comparativa entre algoritmos exatos e heurísticos para resolução do problema NP-completo de Coloração de Vértices em Grafos. Através da implementação de um algoritmo de força bruta e da heurística de Welsh-Powell, demonstramos empiricamente o trade-off fundamental entre otimalidade e eficiência computacional.

\vspace{0.3cm}

\textbf{Força Bruta — Complexidade Exponencial:} Os experimentos confirmaram empiricamente a complexidade exponencial teoricamente predita. O tempo de execução cresce dramaticamente com o tamanho da instância: de microsegundos para 5 vértices até horas para 15 vértices. A extrapolação dos dados indica que instâncias com apenas 20 vértices exigiriam séculos de processamento. Este método, portanto, é limitado a problemas muito pequenos, viável apenas para validação teórica e instâncias com no máximo ~13 vértices.

\vspace{0.3cm}

\textbf{Welsh-Powell — Eficiência Prática:} A heurística demonstrou desempenho excepcional em instâncias de grande porte. A instância com 4\,730 vértices foi processada em apenas 51 milissegundos, tornando o método viável para aplicações práticas reais. Os resultados obtidos (26, 54, 23, 10 e 56 cores para instâncias a-e respectivamente) evidenciam que a heurística produz colorações razoáveis, mesmo sem garantia de otimalidade.

\vspace{0.3cm}

\textbf{Propriedades Estruturais como Fator Determinante:} Uma descoberta importante foi que a densidade do grafo é mais relevante que o tamanho absoluto para determinar o número de cores necessário. A instância \textbf{d}, com 1\,916 vértices mas densidade extremamente baixa (0.68\%), exigiu apenas 10 cores, enquanto a instância \textbf{c}, com apenas 1\,000 vértices mas densidade 2.88\%, necessitou de 23 cores — mais do que o dobro. Esta observação reforça a teoria de grafos aleatórios esparsos e valida a seleção de instâncias com diferentes características estruturais.

\vspace{0.3cm}

\textbf{Conclusão Final:} Para problemas NP-completos como Coloração de Vértices, métodos heurísticos bem projetados são não apenas preferíveis, mas essenciais para resolver instâncias reais. Enquanto a força bruta garante otimalidade, sua complexidade exponencial a torna computacionalmente inacessível para problemas práticos. Welsh-Powell oferece o equilíbrio ideal: implementação simples, complexidade polinomial $O(n \log n + m)$, e capacidade de lidar com grafos de milhares de vértices. Este trabalho reforça um princípio fundamental da Ciência da Computação: em muitos casos, soluções boas obtidas rapidamente são infinitamente mais valiosas que soluções ótimas que nunca podem ser calculadas.
\newpage
% ============================================================================
% REFERÊNCIAS
% ============================================================================
