...
% Definição extrema: garante que o comando existe antes de qualquer pacote
\providecommand{\htmladdnormallink}[2]{#2}

% Corrige erro do comando \htmladdnormallink gerado pelo abnTeX2 (garante redefinição no início do documento)
\usepackage{url}
...
\usepackage{float}                  % Permite usar a opção [H] em figuras
...
% Corrige erro do comando \htmladdnormallink gerado pelo abnTeX2
\usepackage{url}
...
% Redefine \htmladdnormallink para garantir compatibilidade com abnTeX2
\renewcommand{\htmladdnormallink}[2]{\url{#2}}
...
\imprimirreferencias{referencias.bib}                   % Referências (comentado para evitar duplicidade)
...
% Para citar uma referência, use o comando \cite{exemplo2020}
% Adicione o comando abaixo no local desejado do texto:
% \cite{exemplo2020}

% % Configuração da bibliografia (antes do \end{document})
