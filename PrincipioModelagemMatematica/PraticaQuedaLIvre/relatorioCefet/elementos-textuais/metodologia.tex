...
Para cada simulação realizada, os parâmetros de entrada utilizados estão apresentados a seguir:

\textbf{Queda Livre Simples:} Os experimentos deste cenário utilizaram os seguintes parâmetros:
\input{./tabelas/input/parametros_sem_resistencia.tex}

\textbf{Resistência Linear Simplificada:} Os experimentos deste cenário utilizaram os seguintes parâmetros:
\input{./tabelas/input/parametros_linear_simplificada.tex}

\textbf{Resistência Linear Completa (Lei de Stokes):} Os experimentos deste cenário utilizaram os seguintes parâmetros:
\input{./tabelas/input/parametros_linear_completa.tex}

\textbf{Resistência Quadrática Simplificada:} Os experimentos deste cenário utilizaram os seguintes parâmetros:
\input{./tabelas/input/parametros_quadratica_simplificada.tex}

\textbf{Resistência Quadrática Completa:} Os experimentos deste cenário utilizaram os seguintes parâmetros:
\input{./tabelas/input/parametros_quadratica_completa.tex}


\section{Método de Simulação}

Essa estrutura permite rastreabilidade, comparação entre modelos e reprodutibilidade científica.

\section{Observação dos Resultados, Cálculos Analíticos, Análise de Acurácia e Estatística}
\label{sec_analise_resultados}

Esta seção reúne toda a análise dos resultados, os cálculos analíticos, a avaliação da acurácia dos métodos numéricos utilizados e a análise estatística dos experimentos. Para avaliar a acurácia dos métodos numéricos empregados, utilizou-se como referência a solução analítica da equação do movimento para cada caso físico considerado (queda livre simples, resistência linear e quadrática). A solução analítica fornece valores exatos de posição e velocidade ao longo do tempo, permitindo o cálculo do erro relativo entre o resultado numérico e o analítico. Assim, a análise de acurácia baseia-se na comparação direta entre as soluções, sem a necessidade de discutir explicitamente a velocidade terminal.

\textbf{Parâmetros utilizados na análise de acurácia:}
\input{./tabelas/input/entrada_acuracia.tex}
\input{./tabelas/input/parametros_acuracia_sem_resistencia.tex}
\input{./tabelas/input/parametros_acuracia_linear_simplificada.tex}

As principais equações utilizadas foram:
...
\begin{itemize}
    ...
    \begin{equation}
        ...
        v_{\text{final}} = \left(V + \frac{g}{c}\right) \cdot e^{-c t} - \frac{g}{c}
    \end{equation}
