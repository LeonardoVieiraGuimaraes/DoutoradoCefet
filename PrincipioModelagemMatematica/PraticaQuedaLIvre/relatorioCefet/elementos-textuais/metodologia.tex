...
\begin{description}
    ...
    \input{./tabelas/input/parametros_queda_livre.tex}
    ...
    \input{./tabelas/input/parametros_linear.tex}
    ...
    \input{./tabelas/input/parametros_quadratica.tex}
\end{description}
...
\input{./tabelas/input/parametros_acuracia.tex}
...
A metodologia adotada neste trabalho proporcionou uma estrutura robusta e reprodutível para a análise computacional da queda livre sob diferentes regimes físicos. A organização dos experimentos, a escolha criteriosa dos parâmetros e a utilização de métodos numéricos consagrados permitiram avaliar, de forma sistemática, a influência de fatores como resistência do ar e passo de tempo na precisão dos resultados. O detalhamento dos procedimentos e a disponibilização do código-fonte garantem transparência e facilitam a replicação dos experimentos por outros pesquisadores, fortalecendo o rigor científico do estudo.

Essas fórmulas foram implementadas em funções Python para automatizar o cálculo da acurácia dos métodos numéricos. A análise foi realizada considerando a variação do erro relativo médio em função do passo de tempo ($\Delta t$), permitindo identificar a convergência e a precisão do método empregado. Foram gerados gráficos dos principais resultados, incluindo posição, velocidade, aceleração e erro relativo em função de $\Delta t$.

Esses resultados permitiram visualizar o comportamento do sistema sob diferentes condições e validar os modelos computacionais empregados. A análise estatística envolveu a comparação entre as soluções numéricas (método de Euler) e as soluções analíticas para cada modelo físico, avaliando o impacto do passo de tempo na precisão dos resultados e possibilitando a análise comparativa entre os diferentes regimes de resistência.
